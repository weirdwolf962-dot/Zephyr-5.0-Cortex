...
const Icons = {
  ...
  ChevronRight: () => <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>,
};
...
const HistorySidebar = ({ isOpen, onClose, history, onLoadChat, onDeleteChat, onNewChat, generatedImages, onDeleteImage }: {
  ...
  const filteredHistory = history
    ...
    .filter(({ chat }) => {
        ...
        const title = chat[0]?.id === 'refined-title' ? chat[0].text : (firstUserMsg?.text || (firstUserMsg?.image ? "Image uploaded" : "New Conversation"));
                    ...
                    filteredHistory.map(({ chat, index }) => (
                        <div key={index} className="group flex items-center gap-0.5">
                            <button onClick={() => onLoadChat(index)} className="flex-1 flex items-center p-2 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-all text-left group-hover:translate-x-0.5">
                                ...
                                {chat[0]?.id === 'refined-title' ? chat[0].text : (chat.find(m => m.role === Role.USER)?.text || (chat.find(m => m.role === Role.USER)?.image ? "Moment" : "Untitled"))}
                                </span>
                            </button>
...
const App = () => {
  ...
  const startNewChat = () => { archiveCurrentChat(); setMessages([]); setAttachment(null); setInput(''); setIsHistoryOpen(false); setIsDeepSearch(false); };
  ...
  const refineHeadline = async (firstUserText: string, firstModelText: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Create a very short (max 4 words), catchy, and relevant title for a chat that started with: "${firstUserText}". Summarize the essence. Respond only with the title string.`,
      });
      const refinedTitle = response.text?.trim().replace(/"/g, '') || firstUserText.substring(0, 20);
      setMessages(prev => {
        if (prev.length > 0 && prev[0].id === 'refined-title') return prev;
        return [{ role: Role.MODEL, text: refinedTitle, id: 'refined-title' }, ...prev];
      });
    } catch (e) {
      console.warn("Headline refinement failed", e);
    }
  };
  ...
  const sendMessage = async (messageText: string, imageAttachment?: { data: string; mimeType: string; url: string }) => {
    ...
    // Chart-based Model Selection Logic
    let modelToUse = 'gemini-2.5-flash'; // Normal (automatic)
    if (selectedAgent) {
      if (selectedAgent.name === 'News Agent') modelToUse = 'gemini-2.5-flash';
      else if (selectedAgent.name === 'Science Agent') modelToUse = 'gemini-2.5-pro';
      else if (selectedAgent.name === 'Coder Agent') modelToUse = 'gemini-2.5-pro';
      else if (selectedAgent.name === 'Creative Agent') modelToUse = 'gemini-2.5-flash';
    } else {
      if (isDeepSearch) modelToUse = 'gemini-2.5-pro';
    }
    ...
    try {
        ...
        // Filter history, excluding internal state like refined titles
        const historyForAPI = messages
          .filter(m => !m.isLoading && m.id !== 'refined-title')
        ...
        // Headline Refiner Logic after first response
        if (messages.length === 0 || (messages.length === 1 && messages[0].role === Role.USER)) {
          refineHeadline(messageText, fullText);
        }
    ...
    <div className="bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 h-screen flex flex-col font-sans transition-all duration-500 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-8 scroll-smooth">
        <div className="max-w-6xl mx-auto space-y-8 pb-4">
          {messages.filter(m => m.id !== 'refined-title').length === 0 ? (
             <div className="relative flex flex-col items-center justify-center min-h-[60vh] animate-fade-in">
              ...
              <div className="flex flex-col gap-6">
                {messages.filter(m => m.id !== 'refined-title').map((msg, index) => (
                    <ChatMessage 
                      key={msg.id || index} 
                      msg={msg} 
      ...
      <form onSubmit={handleSubmit}
            <div className="relative flex items-center gap-2 px-3 py-2 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900>
              {/* TEXT INPUT */}
               <input
                 type="text"
                 value={input}
...
<div className="flex items-center gap-1.5">
  ...
  <button
    ...
    disabled={isProcessing}
    className={`
      p-3 rounded-xl transition-all active:scale-[0.85]
...
<div
  className="
    mt-4 px-4 text-center text-[11px]
    font-medium text-blue-400 opacity-80
    drop-shadow-[0_0_6px_rgba(59,130,246,0.6)]
    hover:opacity-100 transition-opacity
    ">Zephyr can make mistakes. Check important information.</div>
   </div>
  </div>
 </div>
</div>
